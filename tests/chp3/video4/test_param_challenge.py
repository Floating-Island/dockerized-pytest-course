...
@pytest.mark.parametrize("country,statistic,expectedValue", [
    ('Andorra', 'Mean', 1641.42),
    ('Andorra', 'Median', 1538.02),
    ('Argentina', 'Median', 125.0)
])
def test_csv_writer(process_data, country, statistic, expectedValue):
    """
    ...
    country_statistic_res = data_aggregator.altitude_stat_per_country(data, country, statistic)
    output_location = StringIO()
    data_aggregator.csv_writer(country_statistic_res, output_location)

    res = output_location.getvalue().strip('\r\n')
    assert res == f"Country,{statistic}\r\n{country},{expectedValue}"
